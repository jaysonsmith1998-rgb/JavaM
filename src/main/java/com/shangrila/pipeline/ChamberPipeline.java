...
public final class ChamberPipeline {
    ...
    /**
     * Write {@code shangri_la:cherry_cavern} into the chunk's biome storage
     * for every (qx, qy, qz) quart position that lies inside the carved
     * volume. Biome storage is at 4-block resolution, so we sample one quart
     * cell per 4 blocks.
     */
    private static void setBiome(ServerLevel level, LevelChunk chunk,
                                  RegionContext region, Holder<Biome> caveBiome) {
        // ChunkAccess exposes setBiome(qx, qy, qz, Holder<Biome>) on each
        // section. We use the chunk-level helper.
        int minX = chunk.getPos().getMinBlockX();
        int minZ = chunk.getPos().getMinBlockZ();
        int minQX = minX >> 2;
        int minQZ = minZ >> 2;

        for (int qdx = 0; qdx < 4; qdx++) {
            for (int qdz = 0; qdz < 4; qdz++) {
                int qx = minQX + qdx;
                int qz = minQZ + qdz;
                int blockX = (qx << 2) + 2;       // center of the quart cell
                int blockZ = (qz << 2) + 2;
                if (!ShangriLaRegion.horizontalContains(blockX, blockZ, region.salt())) continue;

                for (int qy = ShangriLaRegion.CHAMBER_Y_BOTTOM >> 2;
                        qy <= ShangriLaRegion.CHAMBER_Y_TOP >> 2; qy++) {
                    int blockY = (qy << 2) + 2;
                    if (ShangriLaRegion.isOpen(blockX, blockY, blockZ, region.salt())) {
                        BiomeWriter.setBiomeAtQuart(chunk, qx, qy, qz, caveBiome);
                    }
                }
            }
        }
